...
Domoticz.prototype.logMessage = function (param, callback){
	var url = this._getUrl();
	url.addSearch("type", "command");
	url.addSearch("param", 'addlogmessage');
	url.addSearch("message", param.message);
	this._request(url, callback);
};
...
Domoticz.prototype.uDevice = function (params, callback) {
	var url = this._getUrl();
	url.addSearch("type", "command");
...
Domoticz.prototype.changeSwitchState = function (params, callback) {
    ...
    params.type === 'dimmable' ? (params.type = 'switch' && params.state === 'Set%20Level&level=' + parseInt((parseInt(params.state) / 100) * 16)) : this;
